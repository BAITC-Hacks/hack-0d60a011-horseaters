...
Клиентская часть системы управления запасами и расчёта закупок. Next.js App Router, TypeScript, TanStack Query v5, Zustand v5, React Hook Form, Zod и Tailwind CSS.
...
Откройте `http://localhost:3000`. По умолчанию включён демо-режим: таблица, фильтры, редактирование параметров расчёта, выбор строк и CSV-выгрузка работают без бэкенда. Изменения демо-данных хранятся в памяти вкладки и сбрасываются при перезагрузке.
