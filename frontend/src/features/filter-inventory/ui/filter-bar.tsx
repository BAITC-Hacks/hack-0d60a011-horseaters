...
type Props = { value: InventoryFilter; onChange: (next: InventoryFilter) => void; suppliers: string[]; count: number };
...
export function FilterBar({ value, onChange, suppliers, count }: Props) {
  ...
  return <div className="flex flex-col gap-4 border-b border-border p-4 lg:flex-row lg:items-center lg:justify-between lg:px-5">
    <div className="flex flex-1 flex-wrap items-center gap-2.5">
      ...
      <div className="relative"><SlidersHorizontal className="pointer-events-none absolute left-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><select aria-label="Статус запаса" value={value.status} onChange={(event) => change({ status: event.target.value as InventoryFilter["status"] })} className={`${selectClass} pl-9`}><option value="all">Все статусы</option><option value="critical">Критический</option><option value="low">Низкий</option><option value="healthy">В норме</option></select></div>
      ...
      {(value.status !== "all" || value.supplier !== "all") && <button onClick={() => onChange({ search: value.search, status: "all", supplier: "all" })} className="text-xs font-medium text-muted-foreground transition hover:text-foreground">Сбросить</button>}
    </div>
