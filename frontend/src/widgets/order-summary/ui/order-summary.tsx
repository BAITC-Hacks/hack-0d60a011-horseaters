...
import { ArrowUpRight, Download, Info, ShoppingCart, Truck } from "lucide-react";
...
export function OrderSummary({ items }: { items: InventoryItem[] }) {
  ...
  const total = orderItems.reduce((sum, item) => sum + (quantityById[item.id] ?? getSuggestedQuantity(item)) * item.unitCost, 0);
  ...
  return <Card id="order-summary" className="overflow-hidden">
    ...
    <div className="p-5">
      ...
      <Button className="w-full" onClick={download} disabled={orderItems.length === 0}><Download className="h-4 w-4" />Скачать CSV</Button>
      ...
      <p className="mt-4 flex items-start gap-2 text-[11px] leading-5 text-muted-foreground"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />Можно изменить количество прямо в таблице. Выбор строк ограничивает состав экспорта.</p>
    </div>
  </Card>;
}
