...
* MVP-параметры расчета сейчас: 365 дней истории, прогноз на 30 дней, страховой запас на 7 дней. Это явные временные допущения, а не утвержденные бизнес-настройки.
* Компенсация дефицита использует явные `stockout_periods`. Автоматическое восстановление периодов отсутствия товара из месячных остатков пока не реализовано.
* Рост берется из `growth_assumptions`; автоматическое обнаружение устойчивого тренда по продажам пока не реализовано. IQR применяется к отдельным положительным транзакциям в рамках артикула/склада; группировку разовой сделки по обезличенному клиенту/документу ещё нужно добавить.
...
* APP-03—APP-05 пока реализованы как application use cases и SQLAlchemy-репозитории. HTTP-маршруты для них — отдельный следующий шаг; `README.md` по просьбе пользователя не изменять.
